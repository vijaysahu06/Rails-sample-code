class Event < ApplicationRecord
  ...
  # scope :addresses, -> { where(addressable_type: 'Event')}
  # scope :addressable_type, -> { where{self.address.addressabe_type 'Event'} }
  # model validations
  ...
  #has_one :creator_user, :foreign_key => :creator_user_id, :class_name => 'User'
  ...
  has_many :attachments, lambda { order('id DESC').limit(1) }, as: :attachable
  has_one :attachment
  ...
  belongs_to :event_cancellation_plan
  has_many :requested_payment_refunds, lambda { PaymentRefund.requested }, class_name: 'PaymentRefund', foreign_key: 'event_id'
  ...
  #model callbacks
  # after_create :create_event_ticket
  after_validation :validate_format_of_registrations_recipients
  validates :registrations_recipients, presence: true, if: :is_event?
  ...
  enum entity_type:{product: 1}
  enum status:{proposed: 0, approved: 1, cancelled: 2, test_registration: 3, ready: 4, suspended: 5, closed: 6}
  include AASM
  ...
  def create_bhandara_details
    if self.bhandara_detail.nil?
      bhandara_params = {budget: 0, event_id: self.id}
      self.create_bhandara_detail(bhandara_params)
    end
  end
  ...
  def get_registration_center_user_id(user_id)
    ...
    return registration_center_user.try(:id)
  end

  def assign_ds_shop_to_event
    logger.info self.ds_shops
    if !self.ds_shops.nil?
      ds_shop_params = {name: "#{self.event_name} shop", description: self.description, event_id: self.id, committee_id: self.committee_id}
  ...
  def self.preloaded_data
    return self.includes(includable_date)
  end
  ...
  def proceed_for_bulk_upload(params)
	...
	def generate_and_upload(options = {})
		...
		# Upload valid absent list to S3
		file = self.generate_excel_file(rows: options[:rows], header: options[:header], data_type: options[:data_type])
	...
	# Method to generate event summary report
	def generate_summary_report
		begin
			addresses = []

			details = Event.where('status IN (?) AND event_end_date + 10 >= ?', Event.statuses.slice(:ready, :test_registration).values, Date.today).where('event_type_id > ?', 0).joins(:address).where.not(addresses: {country_id: 113}).includes({address: [:db_country]}, :event_type, :event_seating_category_associations, :valid_event_registrations).uniq.group_by(&:graced_by).sort

			# To store event addresses
			details.each do |country, events|
				(events || []).each do |e|
			...
			begin
				...
				logger.info("Event: generate_summary_report: emails are: #{emails}")
				if emails.present?
					res = ApplicationMailer.send_email(from: 'support@absclp.com', recipients: emails.split(','), template: 'event_summary_report', subject: "Global Event(s) Summary Report - #{Time.now.strftime('%d%m%Y%H%M%S%N')}", event_details: details, address_details: addresses).deliver
					logger.info("Event: generate_summary_report: Email Sent with response:\n#{res}")
				else
					logger.info('Event: generate_summary_report: No email found.')
				end
			rescue Exception => e
				logger.info("Event: generate_summary_report: Error in sending email: Error: #{e.message}")
				logger.info e.backtrace
				GlobalPreference.notify_exception(e)
			end
		rescue Exception => e
			logger.info("Event: generate_summary_report: Somethig went wrong inside generate_summary_report: Error: #{e.message}")
			logger.info e.backtrace
			GlobalPreference.notify_exception(e)
		end
	end

	#METHOD TO GENERATE iNDIA EVENT REPORTS
	def generate_india_event_reports
		begin
			addresses = []

			details = Event.where('status IN (?) AND event_end_date + 10 >= ? AND entity_type IS ?', Event.statuses.slice(:ready, :test_registration).values, Date.today, nil).where('event_type_id > ?', 0).joins(:address).where('addresses.country_id = ?', 113).includes({address: [:db_country]}, :event_type, :event_seating_category_associations, :valid_event_registrations).uniq.group_by(&:graced_by).sort
			...
			begin
				...
				logger.info("Event: generate_summary_report: emails are: #{emails}")
				if emails.present?
					res = ApplicationMailer.send_email(from: 'registration@shivyogindia.com', recipients: emails.split(','), template: 'india_event_report', subject: "India Event(s) Summary Report - #{Time.now.strftime('%d%m%Y%H%M%S%N')}", event_details: details, address_details: addresses).deliver
					logger.info("Event: generate_summary_report: Email Sent with response:\n#{res}")
				else
					logger.info('Event: generate_summary_report: No email found.')
				end
	...
	def get_clp_detail
		info = {is_clp_event: false, validity_days: 0, type: nil}
	...
	def self.generate_event_excel_data(events)
		...
		events.each do |event|
			...
			# EVENT_START_DATE
			row.push(event.event_start_date.try(:strftime, '%b %d, %Y'))

			# EVENT_START_TIME
			row.push(event.event_start_time)

			# EVENT_END_DATE
			row.push(event.event_end_date.try(:strftime, '%b %d, %Y'))
			...
			# CREATED_AT
			row.push(event.created_at.strftime('%b %d, %Y'))

			# UPDATED_AT
			row.push(event.updated_at.strftime('%b %d, %Y'))
	...
	def process_report_generate(params)
		...
		blob = generate_excel_file(Event.generate_event_excel_data(events))
  ...
  def create_event_order(event_order_params)
    ...
    raise SyException, 'Please input some sadhak profiles for registrations' unless event_order_params[:event_order_line_items_attributes].present?
    raise SyException, 'Please input some sadhak profiles for registrations' if event_order_params[:event_order_line_items_attributes].values.size == 0
    ...
    # valid_profiles_to_register
    event_order_hash = {event_id: id, user_id: event_order_params[:current_user].try(:id), guest_email: guest_email, sy_club_id: event_order_params[:sy_club_id]}
  ...
  def refund_all_without_cancellation(user_id)
    begin
      ...
      begin
        from = sender_email(self)
        recipients = [current_user.sadhak_profile.try(:email), current_user.email].extract_valid_emails.uniq
        ApplicationMailer.send_email(recipients: recipients, subject: "Event Refunds without Cancellation - #{event_name} - #{id}", attachments: { "#{id}_#{event_name.parameterize.underscore}_refunded_details_without_cancellation.xls" => generate_excel_file(header: header, rows: rows)}).deliver if recipients.size.nonzero?
      rescue Exception => e
        notify_exception(e)
      end
    rescue Exception => e
  ...
  def in_delhi?
    event.address.present? && event.address.state_name.to_s.downcase.include?(STATE_DELHI)
  end
